...
import '../../services/interface_auth_service.dart';
import '../../utils/cache/auth_cache_manager.dart';
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this.authService, this.authCacheManager) : super(const AuthState.unknown()) {
    on<AppStarted>((event, emit) async {
      try {
        if (await authCacheManager.isLoggedIn()) {
          ...
          emit((await authCacheManager.isFirstEntry()) ? const AuthState.firstEntry() : const AuthState.guest());
        }
      } on SocketException {
    ...
    on<LoginRequested>(
      (event, emit) async {
        final String? response = await authService.login(email: event.email, password: event.password);
        if (response != null) {
